...
set -v
ARGS="--nodiscover --verbosity 5 --networkid $NETWORK_ID --raft --rpc --rpccorsdomain=* --rpcvhosts=* --rpcaddr 0.0.0.0 --rpcapi admin,db,eth,debug,miner,net,shh,txpool,personal,web3,quorum,raft,zsl --ws --wsaddr 0.0.0.0 --wsapi admin,db,eth,debug,miner,net,shh,txpool,personal,web3,quorum,raft,zsl --wsorigins=* --emitcheckpoints --unlock 0,1 --password passwords.txt $QUORUM_GETH_ARGS"
...
PRIVATE_CONFIG=qdata/c${INDEX_NODE}/tm.ipc nohup geth --datadir qdata/dd ${ARGS} ${permissioned} --raftport ${raftPort} --rpcport ${rpcPort} --port ${port} --wsport ${wsport} 2>>qdata/logs/geth.log &
